...
def write_pipeline_status(step_id, status, stats=None, duration=None):
    """Writes pipeline status to JSON file (called by run_pipeline.py)."""
    ...
    if status_file.exists():
        try:
            with open(status_file, 'r') as f:
                ...
                # Convert to int keys for internal use
                pipeline_status = {int(k): v for k, v in json_status.items() if k.isdigit()}
        except:
            reset_pipeline_status()
            return
    
    # Update status for specific step
    if step_id in pipeline_status:
        pipeline_status[step_id]["status"] = status
        if stats:
            pipeline_status[step_id]["stats"] = stats
        if duration:
            pipeline_status[step_id]["duration"] = duration
    
    # Convert back to string keys for JSON and write
...
def reset_pipeline_status():
    ...
    # Write initial status to file with string keys for JSON compatibility
    status_file = get_pipeline_status_file()
...
def create_pipeline_overview():
    """Creates HTML for pipeline status overview with mobile-responsive design."""
    global pipeline_status
    ...
    for step_id in range(1, 9):
        if step_id in pipeline_status:
            step_info = pipeline_status[step_id]
        else:
            # Default values if step not in status
            step_info = {
                "name": f"Step {step_id}",
                "status": "pending",
                "icon": "📋",
        ...
        if stats:
            ...
            if "qa_pairs" in stats:
                stats_text += f"❓ {stats['qa_pairs']} QA pairs<br>"
            if "questions" in stats:
                stats_text += f"📋 {stats['questions']} questions<br>"
...
def apply_preset(preset_name: str):
    """Applies a finetuning preset and returns the values."""
    ...
    log_message(f"🎯 Applied preset: {preset_name} - {preset['description']}")
    
    return f"✅ Preset '{preset_name}' applied: {preset['description']}"
